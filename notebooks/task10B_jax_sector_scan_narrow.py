...
def plot_sector_suppression(phi_vals, amp2_vals, supp_grid, ratio, fig_dir):
    extent = [phi_vals[0], phi_vals[-1], amp2_vals[0], amp2_vals[-1]]
    vmax = max(0.01, float(np.max(supp_grid)))
    levels = np.linspace(max(0.0, float(np.min(supp_grid))), vmax, 8)
    ...
    if vmax > 1e-8:
        X, Y = np.meshgrid(phi_vals, amp2_vals)
        cs = plt.contour(X, Y, supp_grid, levels=levels, colors="white", linewidths=0.7)
        plt.clabel(cs, inline=True, fontsize=8, fmt="%.3f")
    plt.xlabel(r"$\phi$")
    plt.ylabel(r"$A_2$")
    plt.title(fr"Task 10B narrow: Angular-sector suppression for $\omega_2/\omega_1={ratio:.1f}$")
    plt.tight_layout()
    out = os.path.join(fig_dir, f"task10B_narrow_sector_suppression_ratio_{ratio:.1f}.png")
    plt.savefig(out, dpi=220)
    plt.close()
    print(f"[Saved] {out}", flush=True)


def plot_total_ratio(phi_vals, amp2_vals, Rff_grid, ratio, fig_dir):
    extent = [phi_vals[0], phi_vals[-1], amp2_vals[0], amp2_vals[-1]]
    plt.figure(figsize=(10, 5))
    plt.imshow(
        Rff_grid,
    ...
    plt.xlabel(r"$\phi$")
    plt.ylabel(r"$A_2$")
    plt.title(fr"Task 10B narrow: Total-radiation ratio for $\omega_2/\omega_1={ratio:.1f}$")
    plt.tight_layout()
    out = os.path.join(fig_dir, f"task10B_narrow_total_ratio_ratio_{ratio:.1f}.png")
    plt.savefig(out, dpi=220)
    plt.close()
    print(f"[Saved] {out}", flush=True)
...
def main():
    print("Starting Task 10B narrow-window JAX scan...", flush=True)
    ...
    print(f"Narrow angular window = [{theta_lo:.3f}, {theta_hi:.3f}] rad", flush=True)
    ...
    for ratio in ratio_list:
        ...
        plot_sector_suppression(phi_vals, amp2_vals, sector_supp, ratio, fig_dir)
        plot_total_ratio(phi_vals, amp2_vals, Rff_grid, ratio, fig_dir)
    ...
    print("\nTop 10 candidates by narrow-sector suppression:", flush=True)
    ...
    plt.xlabel(r"$\omega$")
    plt.ylabel(r"$\theta$")
    plt.title("Task 10B narrow: Baseline map")
    plt.tight_layout()
    out = os.path.join(fig_dir, "task10B_narrow_baseline_map.png")
    plt.savefig(out, dpi=220)
    plt.close()
    print(f"[Saved] {out}", flush=True)
    ...
    plt.xlabel(r"$\omega$")
    plt.ylabel(r"$\theta$")
    plt.title("Task 10B narrow: Best candidate map")
    plt.tight_layout()
    out = os.path.join(fig_dir, "task10B_narrow_best_candidate_map.png")
    plt.savefig(out, dpi=220)
    plt.close()
    print(f"[Saved] {out}", flush=True)
    ...
    plt.xlabel("t")
    plt.ylabel("z(t)")
    plt.title("Task 10B narrow: Trajectory comparison")
    plt.legend()
    plt.tight_layout()
    out = os.path.join(fig_dir, "task10B_narrow_trajectory_comparison.png")
    plt.savefig(out, dpi=220)
    plt.close()
    print(f"[Saved] {out}", flush=True)
    ...
    plt.plot(theta_grid, Ibest_profile, label="Best candidate angular profile")
    plt.axvspan(theta_lo, theta_hi, alpha=0.15, label="narrow sector")
    plt.xlabel(r"$\theta$")
    plt.ylabel("Finite-frequency angular intensity")
    plt.title("Task 10B narrow: Angular-profile comparison")
    plt.legend()
    plt.tight_layout()
    out = os.path.join(fig_dir, "task10B_narrow_sector_profile_comparison.png")
    plt.savefig(out, dpi=220)
    plt.close()
    print(f"[Saved] {out}", flush=True)

    print("\nTask 10B narrow complete.", flush=True)
