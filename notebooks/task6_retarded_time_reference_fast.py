import os
import sys
import numpy as np
import matplotlib.pyplot as plt

PROJECT_ROOT = os.path.abspath(os.path.join(os.path.dirname(__file__), ".."))
SRC_PATH = os.path.join(PROJECT_ROOT, "src")
if SRC_PATH not in sys.path:
    sys.path.append(SRC_PATH)

from trajectories import trajectory_sinusoidal
from source_spectrum import compute_Jz_kw


def ensure_dir(path: str) -> None:
    os.makedirs(path, exist_ok=True)


def normalized(arr: np.ndarray) -> np.ndarray:
    arr = np.asarray(arr, dtype=float)
    m = np.max(arr)
    if m <= 0:
        return arr.copy()
    return arr / m


def symmetry_error(theta_grid: np.ndarray, profile: np.ndarray) -> float:
    mirrored = np.interp(np.pi - theta_grid, theta_grid, profile)
    return float(np.max(np.abs(profile - mirrored)))


def solver_profile_single_frequency(
    t: np.ndarray,
    z: np.ndarray,
    vz: np.ndarray,
    omega: float,
    theta_grid: np.ndarray,
    q: float = 1.0,
    c: float = 1.0,
    window: str = "hann",
    normalize_source: bool = True,
) -> np.ndarray:
    """
    Fourier-domain solver profile at one frequency:
        I_spec(omega, theta) ∝ omega^2 sin^2(theta) |J_z(kz_rad, omega)|^2
    """
    prof = np.zeros_like(theta_grid, dtype=float)

    for i, theta in enumerate(theta_grid):
        kz = (omega / c) * np.cos(theta)
        J = compute_Jz_kw(
            t=t,
            z=z,
            vz=vz,
            kz_grid=np.array([kz]),
            omega_grid=np.array([omega]),
            q=q,
            window=window,
            normalize=normalize_source,
        )[0, 0]

        prof[i] = (omega ** 2) * (np.sin(theta) ** 2) * (np.abs(J) ** 2)

    return prof


def reference_profile_single_frequency(
    t: np.ndarray,
    zddot: np.ndarray,
    omega: float,
    theta_grid: np.ndarray,
    window: str = "hann",
) -> np.ndarray:
    """
    Time-domain reference profile at one frequency:
        E_ref(t, theta) ∝ sin(theta) zddot(t)
        I_ref(omega, theta) ∝ |E_ref_tilde(omega, theta)|^2
    """
    if window == "hann":
        w = np.hanning(len(t))
    else:
        w = np.ones_like(t)

    kernel = np.exp(1j * omega * t)
    prof = np.zeros_like(theta_grid, dtype=float)

    for i, theta in enumerate(theta_grid):
        E_t = np.sin(theta) * zddot * w
        E_w = np.trapezoid(E_t * kernel, t)
        prof[i] = np.abs(E_w) ** 2

    return prof


def solver_spectrum_fixed_angle(
    t: np.ndarray,
    z: np.ndarray,
    vz: np.ndarray,
    omega_grid: np.ndarray,
    theta: float,
    q: float = 1.0,
    c: float = 1.0,
    window: str = "hann",
    normalize_source: bool = True,
) -> np.ndarray:
    """
    Fourier-domain solver spectrum at one angle.
    """
    kz_grid = (omega_grid / c) * np.cos(theta)
    Jmat = compute_Jz_kw(
        t=t,
        z=z,
        vz=vz,
        kz_grid=kz_grid,
        omega_grid=omega_grid,
        q=q,
        window=window,
        normalize=normalize_source,
    )
    Jdiag = np.diag(Jmat)
    return (omega_grid ** 2) * (np.sin(theta) ** 2) * (np.abs(Jdiag) ** 2)


def reference_spectrum_fixed_angle(
    t: np.ndarray,
    zddot: np.ndarray,
    omega_grid: np.ndarray,
    theta: float,
    window: str = "hann",
) -> np.ndarray:
    """
    Time-domain reference spectrum at one angle.
    """
    if window == "hann":
        w = np.hanning(len(t))
    else:
        w = np.ones_like(t)

    E_t = np.sin(theta) * zddot * w
    kernel = np.exp(1j * omega_grid[:, None] * t[None, :])
    Ew = np.trapezoid(kernel * E_t[None, :], t, axis=1)
    return np.abs(Ew) ** 2


def main():
    fig_dir = os.path.join(PROJECT_ROOT, "figures")
    ensure_dir(fig_dir)

    q = 1.0
    c = 1.0
    omega0 = 1.5
    d = 0.2
    eps = omega0 * d / c

    T = 80.0 * (2.0 * np.pi / omega0)
    Nt = 3000
    t = np.linspace(-T / 2, T / 2, Nt)

    z, vz = trajectory_sinusoidal(t, d=d, omega0=omega0)
    zddot = -d * (omega0 ** 2) * np.sin(omega0 * t)

    theta_grid = np.linspace(0.0, np.pi, 121)
    omega_grid = np.linspace(-5.0 * omega0, 5.0 * omega0, 601)

    # 1) Fundamental angular profile comparison
    prof_solver = solver_profile_single_frequency(
        t=t, z=z, vz=vz, omega=omega0, theta_grid=theta_grid,
        q=q, c=c, window="hann", normalize_source=True
    )
    prof_ref = reference_profile_single_frequency(
        t=t, zddot=zddot, omega=omega0, theta_grid=theta_grid, window="hann"
    )

    prof_solver_n = normalized(prof_solver)
    prof_ref_n = normalized(prof_ref)

    plt.figure(figsize=(9, 6))
    plt.plot(theta_grid, prof_solver_n, label="Fourier-domain solver")
    plt.plot(theta_grid, prof_ref_n, "--", label="Time-domain reference")
    plt.xlabel(r"$\theta$ [rad]")
    plt.ylabel("Normalized profile")
    plt.title(fr"Task 6 (fast): Fundamental Angular Profile Comparison ($\omega_0 d/c={eps:.3f}$)")
    plt.legend()
    plt.tight_layout()
    out1 = os.path.join(fig_dir, "task6_fast_fundamental_profile_comparison.png")
    plt.savefig(out1, dpi=200)
    plt.close()
    print(f"[Saved] {out1}")

    # 2) Fixed-angle spectrum comparison
    theta0 = np.pi / 2
    spec_solver = solver_spectrum_fixed_angle(
        t=t, z=z, vz=vz, omega_grid=omega_grid, theta=theta0,
        q=q, c=c, window="hann", normalize_source=True
    )
    spec_ref = reference_spectrum_fixed_angle(
        t=t, zddot=zddot, omega_grid=omega_grid, theta=theta0, window="hann"
    )

    spec_solver_n = normalized(spec_solver)
    spec_ref_n = normalized(spec_ref)

    plt.figure(figsize=(9, 6))
    plt.plot(omega_grid / omega0, spec_solver_n, label="Fourier-domain solver")
    plt.plot(omega_grid / omega0, spec_ref_n, "--", label="Time-domain reference")
    for n in range(-5, 6):
        plt.axvline(n, linestyle="--", linewidth=0.8, alpha=0.2)
    plt.xlabel(r"$\omega/\omega_0$")
    plt.ylabel("Normalized spectrum")
    plt.title(r"Task 6 (fast): Fixed-Angle Spectrum Comparison at $\theta\approx\pi/2$")
    plt.legend()
    plt.tight_layout()
    out2 = os.path.join(fig_dir, "task6_fast_fixed_angle_spectrum_comparison.png")
    plt.savefig(out2, dpi=200)
    plt.close()
    print(f"[Saved] {out2}")

    # Diagnostics
    prof_err = float(np.max(np.abs(prof_solver_n - prof_ref_n)))
    sym_solver = symmetry_error(theta_grid, prof_solver_n)
    sym_ref = symmetry_error(theta_grid, prof_ref_n)

    print("\nTask 6 (fast) diagnostics:")
    print(f"Nonrelativistic parameter omega0*d/c = {eps:.6e}")
    print(f"Fundamental profile symmetry error (solver):    {sym_solver:.6e}")
    print(f"Fundamental profile symmetry error (reference): {sym_ref:.6e}")
    print(f"Max normalized profile difference:              {prof_err:.6e}")

    print("\nFixed-angle harmonic comparison at theta ~ pi/2:")
    for n in [1, 2, 3, 4]:
        idx = int(np.argmin(np.abs(omega_grid - n * omega0)))
        print(f"  n = {n}: solver = {spec_solver_n[idx]:.6e}, reference = {spec_ref_n[idx]:.6e}")

    print("\nTask 6 (fast) pass condition:")
    print("1. Solver and reference agree on dominant harmonic structure")
    print("2. Fundamental angular profiles match closely after normalization")
    print("3. Symmetry and on-axis suppression agree")


if __name__ == "__main__":
    main()